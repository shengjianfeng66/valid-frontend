    ...
    <div className="px-6 py-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-gray-600">
            {shouldUseRecommend ? t("users.simulatedUsers.description", { count: simulatedUsers.length }) : ""}
          </span>
        </div>
        {/* 只在未开始状态显示添加按钮 */}
        {shouldUseRecommend && (
          <Button onClick={onAddClick} variant="outline" className="text-gray-600 hover:text-gray-800">
      ...
      {isLoadingUsers && simulatedUsers.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <p className="text-gray-600 mt-4">{shouldUseRecommend ? "正在获取推荐用户..." : "正在获取已访谈用户..."}</p>
        </div>
      ) : (
